...
for (const series of portfolioData.series) {
  ...
  for (let idx = 0; idx < images.length; idx++) {
    ...
    // Tốc độ đàn hồi (lagSpeed) ngẫu nhiên cho từng ảnh để tạo hiệu ứng delay trôi nổi khác nhau khi cuộn
    const lagSpeed = 0.05 + pseudoRandom(seed + 99) * 0.10;
    ...
    const handleResize = () => {
      ...
      layoutPxRef.current = TUNNEL_LAYOUT.map((layout, i) => {
        ...
        return {
          ...
          currentCamZ: existing ? existing.currentCamZ : cameraZRef.current.z
        }
      })
    }
